...
let fillSentence = "Les ______ doivent apprendre à travailler ensemble pour améliorer leurs compétences linguistiques.";
let options = [
  "étudiants",  
  "apprendre", 
  "travailler", 
...
const JumbledSentence = () => {
    ...
    const sentenceParts = fillSentence.split(" ").map((word, index) => {
      ...
      // After finding the blank, start adding words to rightString
      if (!flag) {
        rightString += word + " ";
      }

      // When a word with underscores is found, switch flag to false
      if (word.includes("_")) {
        flag = false;
      }

      return { id: index, type: "word", word };
    });
    ...
    return (
        ...
        <View style={styles.container}>
          ...
          <View style = {styles.sentence}>
            ...
            {
              leftSentenceParts.map((word, index) => (
                <Text key={index} style={styles.wordSentences}>{word}{" "}</Text>
              ))
            }
            <View style = {styles.underline} className='d-flex justify-center align-items-center'>
                { text !== "" &&
                    <TouchableOpacity style = {styles.wordBox}>
                        <Text style = {styles.wordText} className='text-purple-500'>{text}</Text>
                    </TouchableOpacity>
                }
            </View>
            {
              rightSentenceParts.map((word, index) => (
                <Text key={index} style={styles.wordSentences}>{word}{" "}</Text>
              ))
            }
           </View>
